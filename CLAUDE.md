...
make test                # in-process smoke test (16 assertions)
docker compose up --build    # full stack: API + Postgres(pgvector) + Redis
```
Dev login: **visakh@local / learn** (seeded on first start). Hosting (Render + Groq + optional Neon) is in `DEPLOY.md`; one-click via `render.yaml`. The HTML docs are in `docs/` (open `docs/index.html`).
...
- **After changes:** run `make test` (16 assertions) before committing.
